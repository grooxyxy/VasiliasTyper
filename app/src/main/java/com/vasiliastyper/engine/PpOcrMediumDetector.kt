...
/**
 * PP-OCRv6 Medium detector for Android.
 *
 * Expected asset/file layout:
 *   assets/models/ppocrv6_medium_det_onnx/v1/inference.onnx
 *   filesDir/models/ppocrv6_medium_det_onnx/v1/inference.onnx
 *
 ...
 */
object PpOcrMediumDetector {
    private const val TAG = "PpOcrMediumDetector"
    private const val MODEL_KEY = "ppocrv6_medium_det_onnx"
    ...
    fun healthMessage(context: Context): String = when {
        isAvailable(context) -> "PP-OCRv6 Medium ready"
        !initError.isNullOrBlank() -> initError!!
        else -> "PP-OCRv6 Medium unavailable"
    }
        ...
        synchronized(initLock) {
            ...
            return try {
                ...
                if (s != null) {
                    ...
                    initFailed = true
                    initError = "PP-OCRv6 Medium model tidak ditemukan/kompatibel; lihat folder model v1"
                    false
                }
            } catch (t: Throwable) {
                initFailed = true
                initError = t.message ?: t.javaClass.simpleName
                Log.e(TAG, "PP-OCRv6 Medium init failed", t)
                false
            }
        }
    }
            ...
            for (top in starts) {
                ...
                try {
                    val rects = try {
                        ...
                        initError = "PP-OCRv6 Medium inference gagal: ${t.message ?: t.javaClass.simpleName}"
                        Log.e(TAG, "PP-OCRv6 Medium strip inference failed at y=$top", t)
                        emptyList()
                    }
                    for (r in rects) {
                        result.add(
    ...
    private fun cacheAsset(context: Context, replace: Boolean = false): File {
        ...
        if (replace || !out.exists() || out.length() < MIN_MODEL_BYTES) {
            ...
            if (temp.length() < MIN_MODEL_BYTES) {
                temp.delete()
                throw IllegalStateException("Asset PP-OCRv6 Medium tidak lengkap")
            }

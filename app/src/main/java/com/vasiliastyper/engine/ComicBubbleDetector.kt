 ...
 *
 * The supported quantized model is detector_int8.onnx (full RT-DETR-v2 INT8,
 * not the v4-s mobile variant). Its interface is:
...
object ComicBubbleDetector {
    ...
    // A dedicated version prevents the smaller detector-v4-s model from being
    // mistaken for the full RT-DETR-v2 INT8 export. The model is not bundled.
    private const val MODEL_VERSION = 5
    private const val MODEL_NAME = "detector_int8.onnx"
    private const val EXPECTED_MODEL_BYTES = 43_838_857L
    ...
    private fun ensureSession(context: Context): OrtSession? {
        ...
        return try {
            ...
            initialized = true
            Log.i(TAG, "RT-DETR-v2 INT8 comic bubble model aktif: ${modelFile.absolutePath}")
            createdSession
        } catch (error: Throwable) {
    ...
    private fun resolveModelFile(context: Context): File? {
        // Only accept the dedicated versioned location. Do not probe the old
        // unversioned/v4-s locations: both models are valid ONNX files but have
        // different sizes and runtime interfaces.
        val downloadedModel =
            ModelPathRegistry.versionedFile(context, MODEL_KEY, MODEL_VERSION, MODEL_NAME)
        if (isValidModel(downloadedModel)) return downloadedModel
